...
public:
    ...
    virtual int getNodeIndex(Node &node) {
        return 0;
    }
    ...
    std::string outConfusionMatrix(std::vector<int> &groups, map<int,int> &groupLabels) {
        ...
        for (int c = 0; c < confusionMatrix.cols(); c++)
            out << "\tcla" << groupLabels[c];
        out << "\t| Sum" << endl;
        for (int r = 0; r < confusionMatrix.rows(); r++) {
            out << "clu" << r << "\t\t|";
            for (int c = 0; c < confusionMatrix.cols(); c++) {
                out << "\t" << confusionMatrix[r][c];
        ...
        for (int c = 0; c < confusionMatrix.cols(); c++)
            out << "\t" << colSums[c];
        out << "\t| " << colSums.sum() << endl << endl;
        /***/

        out << "Random index: " << ClusteringMetrics::RANDI(confusionMatrix) << endl;
        out << "Adjusted random index: " << ClusteringMetrics::ARI(confusionMatrix) << endl << endl;
        out << "Total noise:\t" << noise << "(" << noise/(float)trainingData->rows() << ")" << endl;
       
        return out.str();
    }
    
    std::string outConditionalEntropy(std::vector<int> &groups) {
        std::stringstream out;
...
class ClusteringMWDSSOM: public ClusteringSOM<MWDSSOM> {

public:
    ClusteringMWDSSOM(MWDSSOM *som) : ClusteringSOM<MWDSSOM>(som){};
